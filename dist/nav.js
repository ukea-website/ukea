...
const rootHref = href => href.startsWith('/') || href.startsWith('#') ? href : `/${href}`;
const link = item => `<a href="${rootHref(item.href)}"${hasCurrent(item) ? ' aria-current="page"' : ''}>${escapeHtml(item.label)}</a>`;
